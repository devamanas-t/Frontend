            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col">
                  <label className="text-xs font-semibold text-neutral-400 mb-2 uppercase tracking-wider">Education Level</label>
                  <div className="relative group">
                    <GraduationCap className="absolute left-4 top-3.5 text-neutral-500 group-hover:text-white transition-colors" size={18} />

                    <select 
                      value={gradeSelect}
                      onChange={handleGradeSelectChange}
                      disabled={isGenerating}
                      className="w-full pl-11 pr-4 py-3 bg-black/50 border border-white/10 rounded-xl text-white appearance-none focus:ring-1 focus:ring-white/30 focus:border-white/30 outline-none transition-all cursor-pointer"
                    >
                      {gradeOptions.map((group, i) => (
                        <optgroup key={i} label={group.label} className="bg-neutral-900 text-white">
                          {group.options.map(option => (
                            <option key={option} value={option} className="bg-neutral-900 text-white">{option}</option>
                          ))}
                        </optgroup>
                      ))}
                      <option value="Other" className="bg-neutral-900 text-white">Other (Type Custom)</option>
                    </select>
                  </div>
